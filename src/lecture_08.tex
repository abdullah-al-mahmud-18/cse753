...
therefore no out-of-distribution query risk at all (During training, not during testing) --- it fully reuses the plain
...
\textbf{Averages treat all mistakes the same.} Imagine guessing a number, where every guess that is too high costs 1 penalty point per unit off, and every guess too low \emph{also} costs 1 penalty point per unit off. Minimizing total penalty over many guesses leads you to the \textbf{average} of all true answers. This is exactly what ordinary least-squares (squared error) loss does.

\textbf{Now change the rules of the game.} Suppose guessing too low costs much more than guessing too high — say, 9 penalty points per unit for undershooting, but only 1 penalty point per unit for overshooting. To minimize total penalty, you will now guess \emph{higher} than the plain average, since low guesses are expensive and you want to play it safe. This shifted, ``leaning'' guess is called an \textbf{expectile}.
...
where $\lambda \in (0,1)$ controls the lopsidedness of the penalty:

\begin{itemize}
    \item $\lambda = 0.5$ \(\rightarrow\) both directions penalized equally \(\rightarrow\) plain average (ordinary squared error).
    \item $\lambda$ close to $0$ \(\rightarrow\) underestimating penalized heavily \(\rightarrow\) fit leans \textbf{up} (optimistic).
    \item $\lambda$ close to $1$ \(\rightarrow\) overestimating penalized heavily \(\rightarrow\) fit leans \textbf{down} (pessimistic).
\end{itemize}

\textbf{Why does anyone want this?} In offline RL, we have many actions someone tried in a state, each with a quality score $Q(s,a)$. We want an estimate of ``how good is the \emph{best} stuff I saw here,'' not the average of everything including mediocre attempts. Penalizing underestimation more heavily (small $\lambda$) pulls the fitted value $V$ up toward the better end of what was actually tried — without ever having to imagine or evaluate an action nobody actually attempted.

\[
\textbf{Expectile} = \text{a mean that has been told to lean toward one side, with } \lambda \text{ as the dial for how hard it leans.}
\]

\paragraph{Why Small $\lambda$ Pulls $V$ Upward, Using Only Real Data}

\subsection{Step 1: What ``Underestimating'' Means Here}

In IQL, we fit a value $V(s)$ to summarize a set of $Q$-values — the scores of actions actually tried in state $s$. The residual is:
\[
x = V(s) - \hat{Q}(s,a)
\]

\begin{itemize}
    \item If $x < 0$: $V(s) < Q(s,a)$ — $V$ is \textbf{underestimating} that action's value.
    \item If $x > 0$: $V(s) > Q(s,a)$ — $V$ is \textbf{overestimating}.
\end{itemize}

\subsection{Step 2: What Small $\lambda$ Does to the Penalty}

Recall: negative $x$ is scaled by $(1-\lambda)$, positive $x$ is scaled by $\lambda$.

For $\lambda = 0.1$:
\begin{align*}
\text{underestimating } (x<0): &\quad \text{multiplier} = 1 - 0.1 = 0.9 \quad (\text{expensive}) \\
\text{overestimating } (x>0): &\quad \text{multiplier} = 0.1 \quad (\text{cheap})
\end{align*}

The loss function effectively says: \emph{``Whatever you do, don't underestimate — that's the costly mistake. Overestimating is fine.''}

\subsection{Step 3: Why This Makes $V$ Climb Upward}

Suppose the observed scores for a state are:
\[
Q = \{2, 4, 6, 8, 10\}
\]

\begin{itemize}
    \item If $V = 6$ (the plain average), $V$ is below three values, each costing the expensive $0.9$ penalty.
    \item If $V = 8$ or $9$, $V$ is below fewer values, and the values it's now above only cost the cheap $0.1$ penalty.
\end{itemize}

Minimizing total penalty naturally drifts $V$ \textbf{upward}, toward the higher-scoring actions — closer to ``what the best observed action was worth,'' rather than the plain average of everything.

\subsection{Step 4: Why This Is Safe}

Every $Q(s,a)$ used above comes from an action \textbf{actually recorded in the data}. No hypothetical or untried action is ever evaluated.

Compare this to the risky approach (plain Q-learning / AWAC), which asks: ``what's the best action the \emph{current policy} would pick right now?'' — even if that action was never tried. This is dangerous, since the model has no real evidence for an untried action's true value, risking wild overestimation.

IQL avoids this entirely:
\[
\text{``Be optimistic about actions you \emph{did} see, by refusing to sell them short — never guess about actions you didn't see.''}
\]

This is the core mechanism behind expectile regression with small $\lambda$: an optimistic value estimate built entirely from real, already-tried actions.
...
\paragraph{Note on the slide}
There are two ways people write the ``lean upward'' rule, and they use opposite
labels for the same idea --- so don't get confused.

\textbf{This slide's way:} it defines the error as
\[
x = V(s) - \hat{Q}(s,a)
\]
So if $x$ is negative, it means $V$ came out \emph{smaller} than $Q$ --- in other
words, $V$ is \emph{underestimating} (selling the action short). The slide says to
use a \emph{small} $\lambda$ (like $\lambda = 0.1$). Remember the rule: negative $x$
gets multiplied by $(1-\lambda)$. If $\lambda$ is small, then $(1-\lambda)$ is close
to $1$ --- a big number --- so underestimating becomes \emph{expensive}. To avoid
that expensive mistake, the best-fit $V$ climbs \emph{upward}. That's exactly the
optimistic behavior we want, and it matches what the slide promised.

\textbf{The original IQL paper's way:} it defines the error the opposite way,
\[
x = \hat{Q}(s,a) - V(s)
\]
and says to use a \emph{large} $\lambda$ (close to $1$) to get the same upward-leaning
effect.
