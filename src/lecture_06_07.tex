...
In plain words: instead of first writing down "how good is each action," and then picking the best one, we build a machine controlled by the dials $\vec\theta$ that spits out action probabilities directly. We slowly turn the dials, a little at a time, so that good behaviour becomes more and more likely. We never need to write a $Q$-value scoreboard to pick an action anymore --- the dials themselves decide.

\paragraph{Aspects of "direct policy parameterization / policy gradient methods."}
\begin{itemize}[leftmargin=*]
  \item \textbf{What it is:} a group of RL methods that tune the policy's own dials directly, by nudging them a little bit in the direction that increases the total reward.
  \item \textbf{Plain explanation:} imagine turning a dial that controls how likely you are to take each possible action. After every attempt, you turn the dial a little bit toward "do more of what worked."
  ...
  \item \textbf{Problem it solves:} it avoids having to search through every action for the best $Q$-value, which value-based methods need and which becomes very hard when there are too many actions or the actions are continuous.
  \item \textbf{Problem it cannot solve by itself:} on its own, it does not fix the "wobbly guess" (high variance) problem, or the problem of figuring out which exact action deserves credit for a good or bad result. Those need extra tricks, covered later in this document.
...
\begin{small}
$$\nabla_\theta J(\theta) = \mathbb{E}_{\tau\sim p_\theta}\left[\Big(\sum_{t=1}^{T}\nabla_\theta\log\pi_\theta(a_t\mid s_t)\Big)\Big(\sum_{t=1}^{T} r(s_t,a_t)\Big)\right] \approx \frac{1}{N}\sum_{i=1}^{N}\Big(\sum_{t=1}^{T}\nabla_\theta\log\pi_\theta(a_{i,t}\mid s_{i,t})\Big)\Big(\sum_{t=1}^{T} r(s_{i,t},a_{i,t})\Big).$$
\end{small}
\begin{center}
...
\textbf{Symbol} & \textbf{What it means} \\
\midrule
$\sum_t \nabla_\theta\log\pi_\theta(a_t\mid s_t)$ & a whole list of numbers (one per dial), pointing in the direction that would make this exact sequence of chosen actions more likely to happen again. \\
$\sum_t r(s_t,a_t)$ & a single number: the total reward earned in that whole run. \\
\bottomrule
\end{tabular}
\end{center}
